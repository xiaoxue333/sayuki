/**
 * Sayuki — ThrownAxeEntity Renderer (renders the axe item stack)
 */
...
public class ThrownAxeRenderer extends EntityRenderer<ThrownAxeEntity> {
    ...
    @Override
    public void render(ThrownAxeEntity entity, float entityYaw, float partialTicks,
                       PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();
        poseStack.mulPose(Axis.YP.rotationDegrees(Mth.lerp(partialTicks, entity.yRotO, entity.getYRot()) - 90.0F));
        poseStack.mulPose(Axis.ZP.rotationDegrees(Mth.lerp(partialTicks, entity.xRotO, entity.getXRot())));
        poseStack.translate(0.0D, 0.15D, 0.0D);
        ItemStack axeStack = entity.getAxeStack();
        if (axeStack == null) axeStack = ItemStack.EMPTY;
        this.itemRenderer.renderStatic(axeStack, ItemDisplayContext.FIXED,
